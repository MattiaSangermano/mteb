...
_EVAL_LANGS = {
        f"{s_lang}-{t_lang}": 
        [_LANGUAGE_MAPPING[s_lang], _LANGUAGE_MAPPING[t_lang]]
    for s_lang in _LANGUAGES for t_lang in _LANGUAGES
    }
    ...
    def load_data(self, **kwargs):
        ...
        for lang_pair in self.hf_subsets:
            ...
            for eval_split in self.metadata.eval_splits:
                ...
                query_ids = {
                    query: f"Q{i}"
                    for i, query in enumerate(set([entry[source_lang_abb] for entry in split_data['queries']]))
                }

                context_texts = set([hit['text'] for entry in split_data["answers"] for hit in entry[target_lang_abb]]) 
                ...
                for row in split_data:
                    ...
                    contexts = [entry['text'] for entry in row["answers"][target_lang_abb]]
